class Container {
  ...
  updateSizeFromDOM() {
    ...
    requestAnimationFrame(() => {
      ...
      if (newWidth === 0 || newHeight === 0) {
        newWidth = this.width || 420
        newHeight = this.height || 280
      }
  ...
  createFallbackTextureSource() {
    ...
    grad.addColorStop(0.3, '#7c3aed') // Violet
    grad.addColorStop(0.65, '#db2777') // Pink
    grad.addColorStop(1, '#ea580c') // Orange/Warm
    ctx.fillStyle = grad
    ...
    ctx.beginPath()
    ctx.arc(380, 380, 180, 0, Math.PI * 2)
    ctx.fill()
    return canvas
  }
  ...
  initWebGL() {
    ...
    if (Container.pageSnapshot) {
      try {
        const dataUrl = Container.pageSnapshot.toDataURL()
        const img = new Image()
        img.src = dataUrl
        img.onload = () => {
        ...
        console.warn('Container.pageSnapshot.toDataURL failed. Using fallback gradient.', e)
      }
    }

    if (!loaded) {
      try {
        const fallback = this.createFallbackTextureSource()
        this.setupShader(fallback)
        this.webglInitialized = true
      } catch (e2) {
        console.error('Failed to setup shader fallback:', e2)
      }
    }
  }
  ...
  `
    ...
    try {
      ...
      console.warn('WebGL manual canvas texture upload blocked. Using procedural fallback.', e)
    }
    ...
    if (!uploaded) {
      try {
        ...
        console.error('WebGL fallback upload failed:', e)
      }
    }

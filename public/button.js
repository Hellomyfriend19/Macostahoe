class Button extends Container {
  ...
  setupDynamicNestedShader() {
    ...
    // Initialize with parent container's current canvas size (fallback to parent defined widths if 0)
    const containerCanvas = this.parent.canvas
    const texWidth = (containerCanvas && containerCanvas.width) || this.parent.width || 420
    const texHeight = (containerCanvas && containerCanvas.height) || this.parent.height || 280
    gl.texImage2D(
      gl.TEXTURE_2D,
      0,
      gl.RGBA,
    ...
    // Set uniforms
    const texWidthVal = (containerCanvas && containerCanvas.width) || this.parent.width || 420
    const texHeightVal = (containerCanvas && containerCanvas.height) || this.parent.height || 280
    gl.uniform2f(resolutionLoc, this.canvas.width || 1, this.canvas.height || 1)
    gl.uniform2f(textureSizeLoc, texWidthVal, texHeightVal)
  ...
  startNestedRenderLoop() {
    const render = () => {
      ...
      // UPDATE TEXTURE FROM PARENT CONTAINER'S CURRENT RENDERED OUTPUT (only if dimensions are valid)
      const containerCanvas = this.parent.canvas
      if (containerCanvas && containerCanvas.width > 0 && containerCanvas.height > 0) {
